...
class Bill_model extends CI_Model {
	...
	public function get_bill_detail_vat_tax($bill_id,$type) {
		//$this->db->delete('bill_detail', array('bill_id' => $bill_id,'type'=>$type));
	...
	public function get_bills(){
		if($this->input->post('from_date')){
			...
			$from_date = date("Y-m-d", strtotime(date('01-m-Y')));
		}
	...
	public function get_vat_tax_total($bill_id) {

        $this->db->select_sum('amount', 'vat_tax_total');

        $query = $this->db->get_where('bill_detail', array('bill_id' => $bill_id,'type'=>'Vat Tax'));

        $row = $query->row();

        return $row->vat_tax_total;
	...
	public function get_bill_payment_data($bill_id){
		$query = $this->db->query('SELECT * FROM '. $this->db->dbprefix('bill_payment_r').' WHERE bill_id='.$bill_id);
		//echo $this->db->last_query($query)."<br/>";
		$result = $query->result_array();
		return $result;
	}
	public function calculate_bill_due_amount($bill_id,$tax_type,$payment_amount){
		$bill_details = $this->get_bill_detail($bill_id);
		$bill_item_amount=0;
		$discount=0;
		$tax_amount=0;
		...
		foreach($bill_details as $bill_detail){
			...
			if($bill_detail['type']!='discount' && $bill_detail['type']!='tax'){
				$bill_item_amount=$bill_item_amount+$bill_detail['amount'];
			}
			
			if($bill_detail['type']=='discount'){
				$discount=$discount+$bill_detail['amount'];
			}
			
			if($tax_type == "item"){
				$item_wise_tax=$item_wise_tax+$bill_detail['tax_amount'];
			}else{
				if($bill_detail['type']=='tax'){
		...
		$due_amount=($bill_item_amount+$tax_amount+$item_wise_tax)-($discount+$payment_amount);
		
		//update bill due amount 
		$sql = "update " . $this->db->dbprefix('bill') . " set  due_amount =  ? where bill_id = ?;";
        $this->db->query($sql, array($due_amount, $bill_id));
