...
class Patient_model extends CI_Model {
	...
	function insert_patient_full($contact_id,$display_id,$reference_by,$gender,$dob,$age=NULL,$ssn_id = NULL){
	...
	function update_patient_full($patient_id,$display_id,$reference_by,$gender,$dob){
    ...
    function get_visits_history($patient_id,$history_from_date,$history_to_date) {

      $this->db->where('visit_date <=',$history_to_date);
      $this->db->where('visit_date >=',$history_from_date);
      $this->db->where('patient_id =',$patient_id);
      $query = $this->db->get('view_visit');
      //echo $this->db->last_query()."<br/>";
      return $query->result_array();
    }
	function get_previous_visits_reports($patient_id,$select_doctors,$visit_from_date,$visit_to_date) {
		$this->db->order_by("visit_date", "desc");
       if (!empty($select_doctors)) {
			$this->db->where_in('doctor_id',$select_doctors);
		}
		if($patient_id != NULL && $patient_id != 0){
			$this->db->where('patient_id',$patient_id);
		}
		$this->db->where('visit_date >=',$visit_from_date);
		$this->db->where('visit_date <=',$visit_to_date);
		$query = $this->db->get('view_visit');
		//echo $this->db->last_query()."<br/>";
		return $query->result_array();
    }
    ...
    public function get_bill($visit_id) {
        ...
        $query = $this->db->get_where('bill', array('bill_id' => $visit_id));
    ...
    public function get_bill_detail($visit_id) {
        ...
        $query = $this->db->get_where('bill_detail', array('bill_id' => $visit_id));
...
//	echo $this->db->last_query()."<br/>";
    ...
    public function delete_bill_detail($bill_detail_id = NULL, $bill_id = NULL) {
		...
		$sql = "update " . $this->db->dbprefix('bill') . " set sync_status = 0,total_amount = total_amount - ?,tax_amount = tax_amount - ? where bill_id = ?;";
        $this->db->query($sql, array($total_amount,$tax_amount, $bill_id));
...
//echo $this->db->last_query();
	...
	function ajax_insert_patient($data) {
		$this->db->insert('patient', $data);
		$p_id = $this->db->insert_id();
		$this->display_id($p_id);
		return $p_id;
