...
*/
	...
	if(isset($payment)){
		...
		$payment_pay_amount = set_value('payment_amount',$payment['pay_amount']);
		$payment_pay_mode = set_value('pay_mode',$payment['pay_mode']);
		$payment_status = $payment['payment_status'];
		$payment_date_value=set_value('payment_date',date($def_dateformate,strtotime($payment['pay_date'])));
		$paid_cash=set_value('paid_cash',$payment['']);
	} else {
		$payment_additional_detail = "";
		$payment_pay_amount =set_value('payment_amount',0) ;
		$payment_pay_mode = set_value('pay_mode',"");
		$payment_status = "";
		//$payment_date_value=set_value('payment_date',"");
		$payment_date_value= set_value('payment_date',date("d-m-Y"));
		$paid_cash=set_value('paid_cash',"");
		$return_change=set_value('return_change',"");
	}
	if(isset($patient)){
...
$(window).load(function(){
	...
	var currencySymbol = "<?=$invoice_settings['currency_symbol'];?>";
	var currencyPostfix = "<?=$invoice_settings['currency_postfix'];?>";
	var decimalSymbol = "<?=$invoice_settings['decimal_symbol'];?>";
	var decimalPlaces = "<?=$invoice_settings['number_of_decimal'];?>";
	var thousandsSeparator = "<?=$invoice_settings['thousands_separator'];?>";
	...
	var in_account_amount_format = currency_format (in_account_amount,currencySymbol,currencyPostfix,decimalPlaces,decimalSymbol,thousandsSeparator);
	$("#adjust_from_account_display").html(in_account_amount_format);
	...
	<?php if(isset($patient_id) && $patient_id !=0 && !isset($payment)){ ?>
			...
			$("#bill_detail_footer").append("<tr><th>Total</th><th style='text-align:right;'>"+currency_format(total_due_amount,currencySymbol,currencyPostfix,decimalPlaces,decimalSymbol,thousandsSeparator)+"<input type='hidden' id='total_due_amount' name='total_due_amount' value='"+total_due_amount+"'/></th><th style='text-align:right;' id='total_payment_amount'></th></tr>");
	<?php }else{ ?>
	...
	function load_bill_table(this_patient_id){
		...
		$("#bill_detail").append("<tr><td>Patient Account</td><td style='text-align:right;'></td><td style='text-align:right;' id='in_account'></td><input type='hidden' id='in_account_amount' name='in_account_amount' value=''/></tr>");
		var total_due_amount_text=total_due_amount;
		var total_due_amount = currency_format(total_due_amount,currencySymbol,currencyPostfix,decimalPlaces,decimalSymbol,thousandsSeparator);
		$("#bill_detail_footer").append("<tr><th>Total</th><th style='text-align:right;'>"+total_due_amount+"<input type='hidden' id='total_due_amount' name='total_due_amount' value='"+total_due_amount_text+"'/></th><th style='text-align:right;' id='total_payment_amount'></th></tr>");
	...
	$("#adjust_from_account").click(function () {
		if ($(this).is(":checked")) {
			...
			$('#total_payment_amount').html(currency_format(payment_amount,currencySymbol,currencyPostfix,decimalPlaces,decimalSymbol,thousandsSeparator));
			...
			if(payment_amount > total_due_amount){
				...
				$('#in_account').html(currency_format(in_account,currencySymbol,currencyPostfix,decimalPlaces,decimalSymbol,thousandsSeparator));
			...
			$('.adjust_amount').each(function(){
				...
				$(this).html(currency_format(adjust_amount,currencySymbol,currencyPostfix,decimalPlaces,decimalSymbol,thousandsSeparator) + '<input type="hidden" name="adjust_amount[]" value="'+adjust_amount+'" />');
			});
			$('#total_due_after_payment').html(currency_format(total_due_after_payment,currencySymbol,currencyPostfix,decimalPlaces,decimalSymbol,thousandsSeparator));
	...
	var searcharrpatient=[<?php $i = 0;
		foreach ($patients as $patient_data) {
			if ($i > 0) { echo ",";}
			echo '[  "' . $patient_data['display_id'] . '","' . $patient_data['first_name'] . " " . $patient_data['middle_name'] . " " . $patient_data['last_name'] . '","' . $patient_data['phone_number'] . '","' . $patient_data['ssn_id'] . '","' . $patient_data['patient_id'].'"]';
			$i++;
		}?>];
	var p_columns = [{name: '<?php echo $this->lang->line("patient").$this->lang->line("id");?>', minWidth:'80px'}, {name: '<?php echo $this->lang->line("name");?>', minWidth:'110px'},  {name: '<?php echo $this->lang->line('phone_number'); ?>', minWidth:'120px'},{name: '<?php echo $this->lang->line('ssn_id'); ?>', minWidth:'100px'},{name: '<?php echo $this->lang->line("id");?>', minWidth: '30px'}];
	...
	$("#patient_name").mcautocomplete({
			...
			$("#adjust_from_account_amount").html(ui.item ? ui.item[5] : '');
			$("#adjust_from_account_display").html(ui.item ? currency_format(ui.item[5],currencySymbol,currencyPostfix,decimalPlaces,decimalSymbol,thousandsSeparator) : '');
			//$("#adjust_from_account_display").html(ui.item ? ui.item[5] : '');

				console.log(ui.item[5]);
				load_bill_table(ui.item[4]);
...
<!-- Page Heading -->
	<div class="card shadow mb-4">
		<div class="card-header py-3 d-sm-flex align-items-center justify-content-between mb-4">
			<h5 class="m-0 font-weight-bold text-primary"><?php echo $this->lang->line("payment_form");?></h5>
		</div>
		<div class="card-body">
			...
			<?php
				...
				}else if(isset($payment_date_value)){
		...
		<?php 
			...
			<div class="row">
				<div class="col-md-6">
					<div class="form-group">
						...
						<input type="text" name="paid_cash" id="paid_cash" class="form-control" value="<?=$paid_cash;?>" />
				...
				<div class="col-md-6">
					<div class="form-group">
						...
						<input type="text" name="return_change" id="return_change" readonly="readonly" class="form-control" value="<?=$return_change;?>" />
					</div>
				</div>
			</div>
			</div>
			<div class="table-responsive-25">
				...
				<div class="row">
					<div class="form-group">
						<?php if(!isset($payment)){ ?>
							<input class="btn btn-primary square-btn-adjust" type="submit" id="save_payment" value="<?php echo $this->lang->line('save');?>" name="submit" />
						<?php }else{ ?>
			...
			<?php if(!isset($payment)){ ?>
				...
				<div class="row">
					<div class="form-group">
							<a href="<?=site_url("payment/index"); ?>" class="btn btn-primary square-btn-adjust btn-sm" ><?php echo $this->lang->line('back_to_payment');?></a>
							<a href="<?=site_url("appointment/index"); ?>" class="btn btn-primary square-btn-adjust btn-sm" ><?php echo $this->lang->line('back_to_appointment');?></a>
					</div>
				</div>
					</div>
				</div>
