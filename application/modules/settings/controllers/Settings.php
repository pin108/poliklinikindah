...
class Settings extends CI_Controller { 
	...
	function upload_new_language_variables_for_language($language){
	...
	public function upload_language_file(){
		...
		//$this->upload_new_language_variables_for_language($language_name);
	...
	public function save_language(){
		...
		}else if (!$this->menu_model->can_access("language",$this->session->userdata('category'))){
			$header_data = get_header_data();
			$this->load->view('templates/header',$header_data);
		 	$this->load->view('templates/menu');
	...
	public function edit_language(){
		...
		public function do_favicon_logo_upload() {
			$config = array();
		...
		public function clinic() {
			if (!$this->session->userdata('user_name') || $this->session->userdata('user_name') == '') {
				redirect('login/index');
			/*}else if ($this->menu_model->has_access("clinic_detail",$this->session->userdata('category'))){
				$this->load->view('templates/access_forbidden');
			*/
			} else {
				...
				if (in_array("centers", $active_modules)) {
					...
					if ($this->form_validation->run() === FALSE) {
						...
						$file_upload_favicon = $this->do_favicon_logo_upload();

						if(isset($file_upload_favicon['error']) && $file_upload_favicon['error']!='<p>You did not select a file to upload.</p>'){
							$data['icon_error'] = $file_upload_favicon['error'];
						}elseif(isset($file_upload_favicon['file_name'])){
							$icon_file_name = $file_upload_favicon['file_name'];
							$this->settings_model->update_clinic_icon($icon_file_name);
						} 
	...
	public function working_days() {
			if (!$this->session->userdata('user_name') || $this->session->userdata('user_name') == '') {
				redirect('login/index');
			} else if (!$this->menu_model->can_access("working_days",$this->session->userdata('category'))){
				$header_data = get_header_data();
				$this->load->view('templates/header',$header_data);
				 $this->load->view('templates/menu');
				$this->load->view('templates/access_forbidden');
		...
		public function save_invoice() {
			if (!$this->session->userdata('user_name') || $this->session->userdata('user_name') == '') {
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$currency_postfix=$this->input->post('currency_postfix');
					$number_of_decimal=$this->input->post('number_of_decimal');
					...
					$config_file = "application/config/config.php";
						...
						for ($i = 0; $i < count($line_array); $i++) {
							if (strstr($line_array[$i], "config['currencySymbol']")) {
								$line_array[$i] = '$config[\'currencySymbol\'] = \'' . $currency_symbol . '\';' . "\r\n";
							}
							if (strstr($line_array[$i], "config['decimalPlaces']")) {
								$line_array[$i] = '$config[\'decimalPlaces\'] = \'' . $number_of_decimal . '\';' . "\r\n";
							}
							if (strstr($line_array[$i], "config['decimalSymbol']")) {
								$line_array[$i] = '$config[\'decimalSymbol\'] = \'' . $decimal_symbol . '\';' . "\r\n";
							}
							if (strstr($line_array[$i], "config['thousandsSeparator']")) {
								$line_array[$i] = '$config[\'thousandsSeparator\'] = \'' . $thousands_separator . '\';' . "\r\n";
							}
							if (strstr($line_array[$i], "config['currencyPostfix']")) {
								$line_array[$i] = '$config[\'currencyPostfix\'] = \'' . $currency_postfix . '\';' . "\r\n";
							}
						}
		...
		public function save_gallery_settings() {
			if (!$this->session->userdata('user_name') || $this->session->userdata('user_name') == '') {
				...
				$this->form_validation->set_rules('allow_file_size', $this->lang->line('allow_file_size'), 'required');
				$this->form_validation->set_rules('allow_file_type', $this->lang->line('allow_file_type'), 'required');
				if ($this->form_validation->run() === FALSE) {
					$this->change_settings();
				} else {
					$this->settings_model->set_data_value("allow_file_size", $this->input->post('allow_file_size'));
					$this->settings_model->set_data_value("allow_file_type", $this->input->post('allow_file_type'));
		...
		public function change_settings() {
			if (!$this->session->userdata('user_name') || $this->session->userdata('user_name') == '') {
				...
				$data['allow_file_size']=$this->settings_model->get_data_value('allow_file_size');
				$data['allow_file_type']=$this->settings_model->get_data_value('allow_file_type');
				...
				$data['patient_account']=$this->settings_model->get_data_value('patient_account');
		...
		public function save_patient_account(){
		//echo $this->input->post('patient_account');
		if($this->input->post('patient_account')){
			$patient_account = 1;
		}else{
			$patient_account = 0;
		}
		$this->settings_model->set_data_value("patient_account", $patient_account);
		redirect("settings/change_settings");
	...
	public function language(){
		if (!$this->menu_model->can_access("language",$this->session->userdata('category'))){
			$header_data = get_header_data();
			$this->load->view('templates/header',$header_data);
			 $this->load->view('templates/menu');
			$this->load->view('templates/access_forbidden');
