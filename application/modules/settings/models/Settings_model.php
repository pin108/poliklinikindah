...
class Settings_model extends CI_Model {
	...
	public function get_loaded_language(){
		$result=$this->db->get_where('language_master',array('is_loaded'=>'1'));
		$languages = $result->result_array();
		//echo $this->db->last_query();
		//print_r($languages);
		return $languages;
	}
	public function get_language_by_file_name($language,$file_name){
		$result=$this->db->get_where('language_data',array('l_name'=>$language,'file_name'=>$file_name));
		$languages = $result->result_array();
		  //echo $this->db->last_query();
		return $languages;
	...
	public function update_clinic_icon($file_name) {

		if($file_name != NULL && $file_name != "" ){

			$data['favicon_logo'] = 'uploads/favicons/'. $file_name;
  ...
  public function delete_language($language_id){
	...
	public function get_tax_rate_by_name($tax_rate_name){

		$query = $this->db->get_where('tax_rates',array('tax_rate_name' => $tax_rate_name));
		//echo $this->db->last_query();
        return $query->row_array();
